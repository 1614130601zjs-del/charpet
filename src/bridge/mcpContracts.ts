import type { DiaryEntry, PetNeeds, PetRecord, PetMood, RelationshipStat, TimelineEntry } from '../pet/petTypes';

/** Stable semantic contract between an AI brain and CharPet's body/life runtime. */
export type CharPetIdentity = {
  charId: string;
  version?: string;
  era?: string;
};

export type CharPetProfile = {
...
export type CharPetNeedMessage = {
  type: 'charpet.need';
  identity: CharPetIdentity;
  need: string;
  intensity: number;
...
export type CharPetEventRequest = {
  type: 'charpet.event.request';
  identity: CharPetIdentity;
  context: {
    time?: string;
    weather?: string;
    ...
    relationship?: RelationshipStat[];
    recentTimeline?: TimelineEntry[];
    recentMemories?: TimelineEntry[];
    items?: string[];
    custom?: Record<string, unknown>;
  };
};

export type CharPetStory = {
  type: 'charpet.story';
  identity: CharPetIdentity;
  id?: string;
  title: string;
  ...
  stateChanges?: Partial<PetNeeds>;
  memory?: string;
  continueFrom?: string;
};

export type CharPetDiary = {
  type: 'charpet.diary';
  identity: CharPetIdentity;
  entry: Omit<DiaryEntry, 'source'> & { source?: 'ai' | 'story' };
};

export type CharPetTimelineAppend = {
  type: 'charpet.timeline.append';
  identity: CharPetIdentity;
  entry: Omit<TimelineEntry, 'id' | 'createdAt'> & { id?: string; createdAt?: number };
};
...
export function buildState(record: PetRecord, emotion: PetMood = 'idle', action = 'idle', isSleeping = false): CharPetState {
  return {
