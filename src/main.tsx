...
function App(){
 ...
 return <main className="app">
  ...
  <section className="hero"><div className="petStage"><div className={`petBubble ${petState.emotion}`}>{petState.speech}</div>{selectedPet?<img src={selectedPet.image} className={`pet ${petState.emotion} action-${petState.action}`} style={{transform:`translate(${petOffset.x}px,${petOffset.y}px)`}} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onClick={interact} draggable={false} alt={selectedPet.name}/>:<div className="placeholder"><div>🐾</div><span>选择一个角色开始</span></div>}</div><div className="panel"><span className="eyebrow">MY PET</span><h2>{selectedPet?.name||'还没有角色'}</h2><p>{selectedPet?`状态：${petState.emotion} · ${petState.action}。点一下它会回应，按住角色可以拖动。`:'你可以直接上传 PNG / JPG / WebP，或者用 SullyOS 捏一个。'}</p><div className="panelActions">{selectedPet&&<button onClick={interact}>逗一下</button>}{selectedPet&&<button onClick={()=>openCreator(selectedPet)}>重新捏</button>}{selectedPet&&<button onClick={()=>{setSelected(null);setPetOffset({x:0,y:0});setPetState(s=>nextIdleState(s))}}>返回角色库</button>}</div></div></section>
