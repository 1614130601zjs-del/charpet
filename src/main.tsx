...
function App() {
  ...
  return <main className="app">
    <header><div><span className="eyebrow">CHARPET STUDIO · V1.0</span><h1>养一只属于你的 Char</h1><p>独立运行、可以捏，也可以直接把自己的角色带进来。</p></div><div className="headerActions"><a className="renderDeploy" href={RENDER_DEPLOY_URL} target="_blank" rel="noreferrer">一键部署到 Render</a><button className="creatorTop" onClick={() => openCreator()}>✦ 捏一个 Char</button><label className="uploadTop">＋ 上传角色<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={e => upload(e.target.files?.[0])} /></label></div></header>
