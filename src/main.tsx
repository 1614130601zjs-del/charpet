...
function App() {
  ...
  function resetTimers() {
    ...
    idleTimer.current = window.setTimeout(() => setPetState(s => nextIdleState(s)), 1200);
    sleepTimer.current = window.setTimeout(() => emit('sleep', 'idle', 0.3, '呼……睡一会儿'), 18000);
  }

  useEffect(() => subscribePetEvents(event => {
    setPetState(s => applyPetEvent(s, event));
    if (event.action !== 'sleep') resetTimers();
  }), []);
  ...
  function interact() {
    if (petState.action === 'sleep') { emit('wake', 'happy', 0.8, '醒啦！'); return; }
    const emotion: PetMood = petState.emotion === 'happy' ? 'surprised' : Math.random() > 0.78 ? 'shy' : 'happy';
    emit('tap', emotion, 0.9, randomLine(emotion));
  }
  function talk() { const emotion: PetMood = Math.random() > 0.7 ? 'shy' : 'happy'; emit('talk', emotion, 0.8, randomLine(emotion)); }
  function mood(emotion: PetMood) { emit('tap', emotion, 0.75, randomLine(emotion)); }
  ...
  return <main className="app">
    <header><div><span className="eyebrow">CHARPET STUDIO · V0.6</span><h1>养一只属于你的 Char</h1><p>独立运行、可以捏，也可以直接把自己的角色带进来。</p></div><div className="headerActions"><button className="creatorTop" onClick={() => openCreator()}>✦ 捏一个 Char</button><label className="uploadTop">＋ 上传角色<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={e => upload(e.target.files?.[0])} /></label></div></header>
    <section className="hero"><div className="petStage"><div className={`petBubble ${petState.emotion}`}>{petState.speech}</div>{selectedPet ? <img src={selectedPet.image} className={`pet ${motion.className}`} style={{ transform: `translate(${petOffset.x}px,${petOffset.y}px)`, scale: motionScale(petState) }} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onClick={interact} draggable={false} alt={selectedPet.name} /> : <div className="placeholder"><div>🐾</div><span>选择一个角色开始</span></div>}</div>
      <div className="panel"><span className="eyebrow">MY PET</span><h2>{selectedPet?.name || '还没有角色'}</h2><p>{selectedPet ? `状态：${petState.emotion} · ${petState.action}。它会自己待机，安静一会儿还会睡觉。` : '你可以直接上传 PNG / JPG / WebP，或者用本地捏人器。'}</p>
        {selectedPet && <div className="panelActions"><button onClick={interact}>逗一下</button><button onClick={talk}>说句话</button><button onClick={() => mood('shy')}>摸摸它</button><button onClick={() => emit('sleep', 'idle', 0.3, '晚安……')}>让它睡</button><button onClick={() => emit('wake', 'happy', 0.8, '早上好！')}>叫醒</button>{selectedPet.source === 'creator' && <button onClick={() => openCreator(selectedPet)}>重新捏</button>}<button onClick={() => { setSelected(null); setPetOffset({ x: 0, y: 0 }); setPetState(s => nextIdleState(s)); }}>返回角色库</button></div>}
