...
function App(){
 ...
 function saveCreator(name:string,state:CreatorState,image:string){const old=selectedPet;const p:Pet={id:old?.id||crypto.randomUUID(),name,image,source:'creator',createdAt:old?.createdAt||Date.now(),creatorState:state};setPets(x=>old?x.map(i=>i.id===old.id?p:i):[p,...x]);setSelected(p);setPetOffset({x:0,y:0});setCreatorOpen(false);emit('wake','happy',0.8,'捏好啦！')}
 ...
 return <main className="app">
  <header><div><span className="eyebrow">CHARPET STUDIO · V0.5</span><h1>养一只属于你的 Char</h1><p>独立运行、可以捏，也可以直接把自己的角色带进来。</p></div><div className="headerActions"><button className="creatorTop" onClick={()=>openCreator()}>✦ 捏一个 Char</button><label className="uploadTop">＋ 上传角色<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>upload(e.target.files?.[0])}/></label></div></header>
  <section className="hero"><div className="petStage"><div className={`petBubble ${petState.emotion}`}>{petState.speech}</div>{selectedPet?<img src={selectedPet.image} className={`pet ${motion.className}`} style={{transform:`translate(${petOffset.x}px,${petOffset.y}px)`,scale:motionScale(petState)}} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} onClick={interact} draggable={false} alt={selectedPet.name}/>:<div className="placeholder"><div>🐾</div><span>选择一个角色开始</span></div>}</div><div className="panel"><span className="eyebrow">MY PET</span><h2>{selectedPet?.name||'还没有角色'}</h2><p>{selectedPet?`状态：${petState.emotion} · ${petState.action}。点一下它会回应，按住角色可以拖动。`:'你可以直接上传 PNG / JPG / WebP，或者用本地捏人器。'}</p><div className="panelActions">{selectedPet&&<button onClick={interact}>逗一下</button>}{selectedPet&&<button onClick={()=>emit('talk','happy',0.8,'你好呀～')}>说句话</button>}{selectedPet&&selectedPet.source==='creator'&&<button onClick={()=>openCreator(selectedPet)}>重新捏</button>}{selectedPet&&<button onClick={()=>{setSelected(null);setPetOffset({x:0,y:0});setPetState(s=>nextIdleState(s))}}>返回角色库</button>}</div></div></section>
  <section><div className="sectionHead"><h2>角色库</h2><span>{pets.length} 个角色</span></div><div className="grid">{pets.map(p=><button className={`card ${selectedPet?.id===p.id?'active':''}`} key={p.id} onClick={()=>{setSelected(p);setPetOffset({x:0,y:0});emit('wake','happy',0.5)}}><div className="thumb"><img src={p.image} alt=""/></div><strong>{p.name}</strong><small>{p.source==='upload'?'图片导入':'本地捏人'}</small></button>)}<button className="addCard" onClick={()=>openCreator()}><span>✦</span><strong>捏一只新的</strong><small>完全本地运行</small></button>{pets.length===0&&<label className="empty">＋<span>也可以上传第一个角色</span><input hidden type="file" accept="image/*" onChange={e=>upload(e.target.files?.[0])}/></label>}</div></section>
